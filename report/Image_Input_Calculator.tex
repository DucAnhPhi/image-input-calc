...
\begin{document}
	...
	Classifying an image means to have a neural network predict how likely the image shows an object from an acquainted catalog, so-called \textit{labels}. As an input, the neural network receives an array including this image's color values. Within the first neuron layer, a non-linear activation function is applied to this input. From there on, the output of each layer is passed as input to the next layer until the output layer is reached (\textit{forward pass}). The final output of the network is a matrix describing the probability for each known class that a data point belongs to that class. Each layer applies it own function to the input what means that each layer has its own matrix of weights that are used to calculate the output. These weight matrices describe the learnable parameters of the network. An untrained network has random weights. Thus, the output it produces is comparable to a guess.
...
%		A future improvement of this project would improve greatly by having an adaptive preprocessing step.
		...
		This happens more often on whiteboards than paper, because they tend to reflect more easily. %Especially, when using images from white boards, the program has to contend with reflected light sources.
	\end{itemize}
		%!!! ??? Add more
...
%	\end{figure}
		...
		\begin{itemize}
			...
			Because of this a new image without this step is created for classification. %??? Errosion is Dilation correct?
%			(Because our image is inverted, the code uses an erosion function.) %??? fix
	...
	\begin{figure*}[htp]
		...
		\subfigure[Border Removal Process]{\includegraphics[width=.45\linewidth]{ImagesForReport/borderMaskWithoutRemoval}}\quad
	...
	%A possible method for doing this is by proposing the y-coordinate position of every line.
	...
	There are other important times where a dot appears in an equation, which unfortunately we were unable to cover, including points in letters (e.g. ``$i=\sqrt{-1}$"), the time derivative of a variable (e.g. ``$\dot{x}=v$") and the dots included in a definition symbol (``$:=$" or ``$=:$").\\  % ??? cite https://en.wikipedia.org/wiki/List_of_mathematical_symbols
...
%The simple preprocessing step performed well on this task.\\
