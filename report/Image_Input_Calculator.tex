...
\section{Segmentation}
	...
	With the assumption that handwritten signs have approximately equal line thickness we filter out contours which are thinner than that.
	...
	These orders have to be taken into account. Additionally fractions have multiple (at least 2) expressions stacked vertically, spanning several lines.
	This can be problematic, when evaluating multiple lines of equations, confusing nominator and denominator for different equations.
	...
	Equal bars are detected similar to the fraction bars with the difference that they only have to contain contours above or below them.
	Once an equal bar is found, we create a bounding box around itself and its counter part (above or below).
